...
$sqlite = '/home/dietpi/teleinfo.sqlite';
 ...
 //
  ...
  function getHPHCSharesByWeek () {
    ...
    $results = $db->query("select strftime('%Y-%W', datetime(timestamp-1, 'unixepoch', 'localtime')) as week, 100*sum(daily_hc)/(sum(daily_hc)+sum(daily_hp)) as shareHC, 100*sum(daily_hp)/(sum(daily_hc)+sum(daily_hp)) as shareHP from conso group by week;");
